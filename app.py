...
from config import INSTANCE_LABEL, PROFILE_PATH
...
def internal_checkin():
    ...
    prompt = (
        "Fais un check-in du matin (bref). Format: bonjour bref + météo (si fournie) "
    ...
    if sid and tok and to:
        try:
            ...
            return (
                jsonify(
                    {"status": "twilio-error", "error": str(e)[:200], "text": text}
                ),
                200,
            )
